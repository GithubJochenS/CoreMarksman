...
namespace CoreMarksman
{
    ...
    public class WindowsSystemTypes : PCSystemTypes
    {
        ...
        {
            SystemTypes = new Dictionary<string, string>();
            SystemTypes.Add("0", "Undefined");
            SystemTypes.Add("1", "Desktop");
            SystemTypes.Add("2", "Laptop");
            SystemTypes.Add("3", "Workstation");
            SystemTypes.Add("4", "Enterprise Server");
            SystemTypes.Add("5", "SOHO Server");
            SystemTypes.Add("6", "Appliance PC");
            SystemTypes.Add("7", "Performance Server");
            SystemTypes.Add("8", "Maximum");
        }
    }
    ...
    public class Sentry // Data acquissition
    {
        ...
        public Sentry(NameValueCollection appSettings) // constructor 
        {
        ...
        {
            string assetLocation = Values["Location"];
        ...
        public Asset GetAsset(NameValueCollection appSettings, SnipeItApi snipe)
        {
            ...
            bool interactiveParseSuccess = bool.TryParse(appSettings["Interactive"], out isInteractive);
        ...
        { // safely addes queries to the queryList monstrocity, built for expandability (c) 
            ...
            if (Queries.ContainsKey(queryType))
            {
                ...
                Queries[queryType] = queryList;
                return;
            }
            else
            {
                queryList.Add(queryString);
                Queries.Add(queryType, queryList);
                return;
            }
        }
        ...
        { // runs all WMI queries
            ...
            foreach (string wmiQuery in Queries["WMI"])
            {
                int count = 0;
        ...
        private void RunLocation() // Runs all code related to location & location sources
        {
            ...
            foreach (string locationQuery in Queries["Location"])
            {
                if (locationQuery == "OU")
                {
                    try
            ...
            Values.Add("Location", location_string);
        ...
        public string GetFormattedVariable(string key, string variable = "", string format = "<name>=<var>") // produces formatted output, supposed to throw exception if no results in raw results
        {
            ...
            if (Values.ContainsKey(key))
            {
                return format.Replace("<var>", Values[key]).Replace("<name>", variable);
            }
            else
            {
    ...
    class Marksman
    {
        ...
        static void Main(string[] args)
        {
            ...
            bool getOU = false;
            bool getOUSuccess = bool.TryParse(appSettings["OUEnabled"], out getOU);
